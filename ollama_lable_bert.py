...
RESULTS_PREFIX = BASE_DIR / 'data' / 'results' / 'ollama_pseudo_label_bert'
SUMMARY_PREFIX = BASE_DIR / 'data' / 'summary'
OUTPUT_PREFIX = BASE_DIR / 'data' / 'hf_dir'
SUMMARY_FILE = SUMMARY_PREFIX / "ollama_pseudo_label_bert_summary.txt"
...
def load_and_prepare(dataset_name, tokenizer):
    ...
    if not train_path.exists():
        raise FileNotFoundError(f"Pseudo-label file not found: {train_path}")
    ...
    if "review_text" not in train_df or "predicted" not in train_df:
        raise ValueError(f"train_df missing columns in {train_path}: got columns {list(train_df.columns)}")
    if "review_text" not in test_df or "polarity" not in test_df:
        raise ValueError(f"test_df missing columns in {test_path}: got columns {list(test_df.columns)}")
    
    # Use predicted labels from LLM for training
    train_df["label"] = train_df["predicted"].astype(int)
...
def train_and_evaluate(model_name, model_checkpoint, dataset_name, summary_file):
    ...
    training_args = TrainingArguments(
        output_dir=str(OUTPUT_PREFIX / f"ollama_pseudo_label_bert_{model_name}_{dataset_name}"),
